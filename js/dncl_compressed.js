/*
 * DNCL Code Generator for Gakushucho (共通テスト用プログラム表記 対応・独自実装エンジン)
 *
 * 【参考文献】
 * 本モジュールは、独立行政法人大学入試センターが公表している共通テスト用プログラム表記の
 * 仕様を参考に、『ブロック学習帳』プロジェクトにおいて独自に設計・実装したコード生成
 * モジュールです。世間の慣例に従い、本プロジェクトでは略称として「DNCL」の呼称を
 * 使用しています。
 */
(function() {
